...
print("=" * 50)
print("ТЕСТ 13: ВХОД С ПУСТЫМ ПОЛЕМ ПАРОЛЯ")
print("=" * 50)
...
try:
    ...
    debug_screenshot = os.path.join(screenshots_dir, f"test_13_debug_{timestamp}.png")
    driver.save_screenshot(debug_screenshot)
    print(f"   Скриншот сохранен: test_13_debug_{timestamp}.png")
    driver.quit()
    exit()

print("\n6. Заполняем форму входа с пустым паролем...")
...
try:
    ...
    print("   ✓ Поле пароля оставлено пустым")
    time.sleep(0.5)
    
    print("\n7. Делаем скриншот заполненной формы...")
    timestamp = datetime.datetime.now().strftime("%Y%m%d_%H%M%S")
    before_login_screenshot = os.path.join(screenshots_dir, f"test_13_before_login_{timestamp}.png")
    driver.save_screenshot(before_login_screenshot)
    print(f"   Скриншот сохранен: test_13_before_login_{timestamp}.png")
    ...
    after_login_screenshot = os.path.join(screenshots_dir, f"test_13_after_login_{timestamp}.png")
    driver.save_screenshot(after_login_screenshot)
    print(f"   Скриншот сохранен: test_13_after_login_{timestamp}.png")
    
    # Выводим итог
    print("\n" + "=" * 40)
    print("РЕЗУЛЬТАТ ТЕСТА 13:")
    print("=" * 40)
    ...
    if login_failed:
        ...
        print("✗ ТЕСТ ПРОВАЛЕН!")
        print("  Вход выполнился (хотя не должен был с пустым паролем)")
        
except Exception as e:
    print(f"   ✗ Ошибка при выполнении теста: {e}")
    
    error_timestamp = datetime.datetime.now().strftime("%Y%m%d_%H%M%S")
    error_screenshot = os.path.join(screenshots_dir, f"test_13_error_{error_timestamp}.png")
    driver.save_screenshot(error_screenshot)
    print(f"   Скриншот ошибки сохранен: test_13_error_{error_timestamp}.png")
...
print("✓ Заполнена форма (телефон + ПУСТОЙ пароль)")
print(f"✓ Сделано 2 скриншота:")
print(f"  - test_13_before_login_{timestamp}.png (перед входом)")
print(f"  - test_13_after_login_{timestamp}.png (после попытки входа)")
print("✓ Нажата кнопка 'Войти'")
