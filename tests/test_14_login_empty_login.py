...
print("=" * 50)
print("ТЕСТ 14: ВХОД С ПУСТЫМ ПОЛЕМ ЛОГИНА")
print("=" * 50)
...
try:
    ...
    debug_screenshot = os.path.join(screenshots_dir, f"test_14_debug_{timestamp}.png")
    driver.save_screenshot(debug_screenshot)
    print(f"   Скриншот сохранен: test_14_debug_{timestamp}.png")
    driver.quit()
    exit()

print("\n6. Заполняем форму входа с пустым логином...")

try:
    # Оставляем поле логина ПУСТЫМ
    login_field.clear()
    print("   ✓ Поле логина оставлено пустым")
    time.sleep(0.5)
    ...
    timestamp = datetime.datetime.now().strftime("%Y%m%d_%H%M%S")
    before_login_screenshot = os.path.join(screenshots_dir, f"test_14_before_login_{timestamp}.png")
    driver.save_screenshot(before_login_screenshot)
    print(f"   Скриншот сохранен: test_14_before_login_{timestamp}.png")
    ...
    after_login_screenshot = os.path.join(screenshots_dir, f"test_14_after_login_{timestamp}.png")
    driver.save_screenshot(after_login_screenshot)
    print(f"   Скриншот сохранен: test_14_after_login_{timestamp}.png")
    
    # Выводим итог
    print("\n" + "=" * 40)
    print("РЕЗУЛЬТАТ ТЕСТА 14:")
    print("=" * 40)
    ...
    if login_failed:
        ...
        print("✗ ТЕСТ ПРОВАЛЕН!")
        print("  Вход выполнился (хотя не должен был с пустым логином)")
        
except Exception as e:
    print(f"   ✗ Ошибка при выполнении теста: {e}")
    
    error_timestamp = datetime.datetime.now().strftime("%Y%m%d_%H%M%S")
    error_screenshot = os.path.join(screenshots_dir, f"test_14_error_{error_timestamp}.png")
    driver.save_screenshot(error_screenshot)
    print(f"   Скриншот ошибки сохранен: test_14_error_{error_timestamp}.png")
...
print("✓ Заполнена форма (ПУСТОЙ логин + пароль)")
print(f"✓ Сделано 2 скриншота:")
print(f"  - test_14_before_login_{timestamp}.png (перед входом)")
print(f"  - test_14_after_login_{timestamp}.png (после попытки входа)")
print("✓ Нажата кнопка 'Войти'")
