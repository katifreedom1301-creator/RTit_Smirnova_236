...
print("=" * 50)
print("ТЕСТ 10: ВХОД ПО НЕВЕРНОЙ ПОЧТЕ НО ВЕРНОМУ ПАРОЛЮ")
print("=" * 50)
...
try:
    ...
    # Берем неверную почту из TEST_DATA
    wrong_email = TEST_DATA["invalid"]["wrong_email"]
    
    # Берем верный пароль из аккаунта acc2
    valid_password = TEST_ACCOUNTS["acc2"]["password"]
    
    print(f"✓ Используем данные:")
    print(f"  Почта: {wrong_email} (неверная)")
...
try:
    ...
    debug_screenshot = os.path.join(screenshots_dir, f"test_10_debug_{timestamp}.png")
    driver.save_screenshot(debug_screenshot)
    print(f"   Скриншот сохранен: test_10_debug_{timestamp}.png")
    driver.quit()
    exit()

print("\n6. Заполняем форму входа с неверной почтой...")

try:
    # Вводим НЕВЕРНУЮ почту
    login_field.clear()
    login_field.send_keys(wrong_email)
    print(f"   ✓ Введена неверная почта: {wrong_email}")
    time.sleep(0.5)
    ...
    timestamp = datetime.datetime.now().strftime("%Y%m%d_%H%M%S")
    before_login_screenshot = os.path.join(screenshots_dir, f"test_10_before_login_{timestamp}.png")
    driver.save_screenshot(before_login_screenshot)
    print(f"   Скриншот сохранен: test_10_before_login_{timestamp}.png")
    ...
    after_login_screenshot = os.path.join(screenshots_dir, f"test_10_after_login_{timestamp}.png")
    driver.save_screenshot(after_login_screenshot)
    print(f"   Скриншот сохранен: test_10_after_login_{timestamp}.png")
    
    # Выводим итог
    print("\n" + "=" * 40)
    print("РЕЗУЛЬТАТ ТЕСТА 10:")
    print("=" * 40)
    ...
    error_screenshot = os.path.join(screenshots_dir, f"test_10_error_{error_timestamp}.png")
    driver.save_screenshot(error_screenshot)
    print(f"   Скриншот ошибки сохранен: test_10_error_{error_timestamp}.png")
...
print("✓ Заполнена форма (неверная почта + верный пароль)")
print(f"✓ Сделано 2 скриншота:")
print(f"  - test_10_before_login_{timestamp}.png (перед входом)")
print(f"  - test_10_after_login_{timestamp}.png (после попытки входа)")
print("✓ Нажата кнопка 'Войти'")
